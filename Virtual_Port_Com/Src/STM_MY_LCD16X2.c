...
void LCD1602_print_double(char string[])
{
	LCD1602_1stLine();
	for(uint8_t i=0;  i< 16 && string[i]!=NULL; i++)
	{
	...
	LCD1602_2ndLine();
	for(uint8_t i=16;  i< 32 && string[i]!=NULL; i++)
	{
		LCD1602_writeData((uint8_t)string[i]);
		HAL_Delay(100);
	}
...
//3.5 LCD animated print
void LCD1602_aprint(char string[])
{
	...
	for(x=0;x<=dupa;x++){
		for(i = 0; i < 16; i++){
			LCD1602_writeData((uint8_t)string[i+x*16]);
			HAL_Delay(250);
		}
		LCD1602_clear();
	}
}
//3.75 print both line
void LCD1602_bprint(char string[])
{
	...
	LCD1602_2ndLine();
	for(uint8_t i=16; i < string[i]!=NULL; i++)
	{
		LCD1602_writeData((uint8_t)string[i]);
	}
...
{
	char numStr[16];
	printf(numStr,"%.*f",decimalPoints, number);
	LCD1602_print(numStr);
}
